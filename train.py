...
def get_dataloaders(train_cfg: config.TrainConfig, vlm_cfg: config.VLMConfig) -> tuple:
    ...
    train_dataset = VQADataset(
        train_ds.select(range(total_samples)), tokenizer, image_processor
    )
...
def train(train_cfg: config.TrainConfig, vlm_cfg: config.VLMConfig):
    ...
    print(
        f"Training summary: {len(train_loader.dataset)} samples, {len(train_loader)} batches/epoch, batch size {train_cfg.batch_size}"
    )
    ...
    for epoch in range(train_cfg.epochs):
        ...
        for batch in train_loader:
            ...
            ):  # Set to float16 if your hardware doesn't support bfloat16ß
                _, loss = model(
            ...
            if train_cfg.eval_in_epochs and global_step % 100 == 0:
                ...
                if epoch_accuracy > best_accuracy:
                    ...
                    print(
                        f"Step: {global_step}, Loss: {batch_loss:.4f}, Tokens/s: {tokens_per_second:.2f}, Accuracy: {epoch_accuracy:.4f} | Saving checkpoint to {vlm_cfg.vlm_checkpoint_path}"
                    )
                else:
                    print(
            ...
            if train_cfg.log_wandb:
                run.log(
                    {"batch_loss": batch_loss, "tokens_per_second": tokens_per_second},
                    step=global_step,
                )
...
def main():
    ...
    parser.add_argument(
        "--vlm_checkpoint_path", type=str, help="Path to the VLM checkpoint"
    )
    parser.add_argument(
        "--resume_from_vlm_checkpoint",
        type=bool,
        default=False,
        help="Resume training from VLM checkpoint",
    )
